...
export const ModalCreatePhieuMua = ({ isOpenCreate, closeModalCreate }) => {
            ...
            setTimeout(() => {
                ...
                if (el) {
                    try { el.focus(); el.select?.(); } catch (error) {
                        console.error("Error focusing/selecting element:", error);
                    }
                }
            }, 150);
        }
    ...
    const hangHoaColumns = [
        ...
        },
        {
            ...
            width: 150,
            render: (val, row) => (
                <div className={`text-gray-800 text-center ${row.id === 'total' ? 'font-bold' : 'font-medium'}`}>
                    {row.ten_vt}
                </div>
            )
        },
