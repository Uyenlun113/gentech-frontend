...
export default function GeneralLedgerUpdate() {
    ...
    useEffect(() => {
        ...
        if (recordData && recordData.phieu) {
            ...
            if (data.hachToan && Array.isArray(data.hachToan) && data.hachToan.length > 0) {
                setHachToanData(data.hachToan.map((item, index) => ({
                    id: index + 1,
            ...
            if (data.hopDongThue && Array.isArray(data.hopDongThue) && data.hopDongThue.length > 0) {
                setHopDongThueData(data.hopDongThue.map((item, index) => ({
                    id: index + 1,
    ...
    const hachToanColumns = [
        ...
        { key: "dien_giaii", title: "Diễn giải", width: 200, render: (val, row) => <Input value={row.dien_giaii} onChange={e => handleHachToanChange(row.id, 'dien_giaii', e.target.value)} /> },
            ...
            <div className="space-y-6">
                ...
                <ComponentCard>
                    <div className="grid grid-cols-1 gap-6 xl:grid-cols-2 mb-6">
                        ...
                        <div><Label>Trạng thái</Label><Select value={trangThai} options={statusOptions} onChange={setTrangThai} /></div>
                    </div>
