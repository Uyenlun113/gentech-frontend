...
import { useUpdatePhieuNhapKho } from "../../hooks/usephieunhapkho";
...
export const ModalEditPhieuNhapKho = ({ isOpenEdit, closeModalEdit, selectedPhieuNhapKho }) => {
  ...
  const { mutateAsync: updatePhieuNhapKho, isPending } = useUpdatePhieuNhapKho();
  ...
  // Load data when selectedPhieuNhapKho changes
  useEffect(() => {
    if (selectedPhieuNhapKho && isOpenEdit) {
      setFormData({
        so_ct: selectedPhieuNhapKho.so_ct || "",
        ong_ba: selectedPhieuNhapKho.ong_ba || "",
        ngay_lct: selectedPhieuNhapKho.ngay_lct ? new Date(selectedPhieuNhapKho.ngay_lct).toLocaleDateString("en-CA") : "",
        ngay_ct: selectedPhieuNhapKho.ngay_ct ? new Date(selectedPhieuNhapKho.ngay_ct).toLocaleDateString("en-CA") : "",
        tk: selectedPhieuNhapKho.tk || "",
        ma_gd: selectedPhieuNhapKho.ma_gd || "",
        ma_kh: selectedPhieuNhapKho.ma_kh || "",
        dia_chi: selectedPhieuNhapKho.dia_chi || "",
        dien_giai: selectedPhieuNhapKho.dien_giai || "",
        ma_qs: selectedPhieuNhapKho.ma_qs || "",
        loai_ct: selectedPhieuNhapKho.loai_ct || "Đã ghi sổ cái",
        mst: selectedPhieuNhapKho.mst || "",
        ma_nt: selectedPhieuNhapKho.ma_nt || "VND",
        ty_gia: selectedPhieuNhapKho.ty_gia || "1",
      });

      // Set search values for existing data
      setMaKhSearch(selectedPhieuNhapKho.ma_kh || "");
      setMaTaiKhoanSearch(selectedPhieuNhapKho.tk || "");
      setMaTaiKhoanSearch2(selectedPhieuNhapKho.tk || "");

      // Load hang_hoa_list data instead of tai_khoan_list
      if (selectedPhieuNhapKho.hang_hoa_list && selectedPhieuNhapKho.hang_hoa_list.length > 0) {
        const hangHoaDataFromServer = selectedPhieuNhapKho.hang_hoa_list.map((item, index) => ({
          id: index + 1,
      ...
      if (selectedPhieuNhapKho.tk && accountData.data) {
        const accountInfo = accountData.data.find(acc => acc.tk === selectedPhieuNhapKho.tk);
        if (accountInfo) {
  ...
  }, [selectedPhieuNhapKho, isOpenEdit, accountData.data, fetchMaterialNames]);
  ...
  const validateForm = useCallback(() => {
    ...
    if (!selectedPhieuNhapKho) {
      console.error("Không có dữ liệu phiếu nhập kho để cập nhật");
      return false;
    }
    ...
    return true;
  }, [formData, hangHoaData, selectedPhieuNhapKho]);
  ...
  const handleEnterPressOnLastField = useCallback((currentRowId) => {
    ...
    setTimeout(() => {
      ...
      const newRowTabIndex = 8 + (hangHoaData.length * 8); // 8 is base tabIndex for ma_vt, 8 fields per row
  ...
  const handleSave = useCallback(async (e) => {
    ...
    try {
      ...
      await updatePhieuNhapKho({
        stt_rec: selectedPhieuNhapKho.stt_rec,
        data: payload
      });
      closeModalEdit();
      resetForm();
      navigate("/chung-tu/phieu-nhap-kho");
    } catch (err) {
      console.error(err);
    }
  }, [formData, hangHoaData, totals, updatePhieuNhapKho, closeModalEdit, resetForm, navigate, validateForm, selectedPhieuNhapKho]);

  return (
    <Modal isOpen={isOpenEdit} onClose={handleClose} title="Cập nhật phiếu nhập kho" className="w-full max-w-7xl m-1 border-2">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                Cập nhật thông tin Phiếu nhập kho trong hệ thống
              </p>
            </div>
          </div>
        </div>
